...
def get_face_landmarks(image, draw=False):
    ...
    if len(image.shape) == 2:
        # Convert grayscale image to RGB by duplicating the grayscale channel
        image_input_rgb = cv2.cvtColor(image, cv2.COLOR_GRAY2RGB)
    else:
        # Assume the image is already in RGB
    ...
    if results.multi_face_landmarks:
        if draw:
            ...
            # Draw landmarks on the original image (convert back to BGR for display)
            image_display = cv2.cvtColor(image_input_rgb, cv2.COLOR_RGB2BGR)
            ...
            cv2.imshow("Landmarks", image_display)
            cv2.waitKey(0)
